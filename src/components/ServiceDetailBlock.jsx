...
export default function ServiceDetailBlock({ category, reverse = false }) {
  if (category.featured) {
    return (
      <div
        id={category.slug}
        className="relative overflow-hidden rounded-lg border border-jc-orange-primary/40 bg-cover bg-center p-8 md:p-12 scroll-mt-36"
        style={bgImage(category.image)}
      >
        <div className="absolute inset-0 bg-gradient-to-r from-jc-black via-jc-black/90 to-jc-black/50" />
        <div className="relative">
          ...
          <div className="grid gap-3 sm:grid-cols-3 mb-6">
            {category.services.map((s) => (
              <div key={s.name} className="rounded-md bg-jc-black/60 p-4 border border-white/10">
